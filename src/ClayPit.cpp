...
ClayPit::ClayPit(int row, int col, int newId, std::string symbol, int health, int level, int maxLevel,
...
IObject* ClayPit::Create(size_t type, IoCContainer *iocContainer, int newId) {
    return new ClayPit(iocContainer->GetY<ClayPit>(), iocContainer->GetX<ClayPit>(),
                     newId, "ClayPit", 10 , 1, 3, iocContainer->Get<Clay>(1)->GetType(), 1, 1.0);
}
